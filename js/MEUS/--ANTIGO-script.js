function validateForm(event) {
    ...
    if (firstname.length < 3) {
        document.getElementById("span4").innerHTML = "O nome é muito curto.";
        isFieldsValidated = false;

    }
    // Validar ultimo nome (pelo menos 3 caracteres)
    if (lastname.length < 10) {
        document.getElementById("span5").innerHTML = " Telefone invalido, tente novamnete.";
        isFieldsValidated = false;
    }
    // Validar email usando expressão regular simples
    if (!emailRegex.test(address)) {
        document.getElementById("span6").innerHTML = 'Email inválido, tente novamnete.';
        isFieldsValidated = false;
    }

  // Validar senha (pelo menos 6 caracteres)
    if (password.length < 6) {
        document.getElementById("span7").innerHTML = 'Senha curta, use 6 caracteres ou mais.';
        isFieldsValidated = false;
    }
...
function validateLOGIN(event) {
    ...
    if (email === "") {
      document.getElementById("span6").innerHTML = "Por favor, preencha o campo de e-mail.";
      isFieldsValidated = false;
    }
